...
async function runTest(browserType) {
    ...
    console.log(chalk.magentaBright.bold(`${argv.capture ? 'Capturing' : 'Running'} Visual Regression Tests (${paramString})...`));
    ...
    if (!argv.capture) {
        ...
        if (!fs.existsSync(snapshotSubDir)) {
            console.error(chalk.red.bold(`Snapshot directory '${snapshotSubDir}' does not exist! Did you forget to run in --capture mode first?`));
            return 1;
        }
    ...
    await page.exposeFunction('snapshot', async (test) => {
        snapshotsTested++;
        const snapshotIndex = (testCounters[test] = (testCounters[test] || 0) + 1);
        const makeFilename = (postfix) => `${test}-${snapshotIndex}${postfix ? `-${postfix}` : ''}.png`;
        const snapshotPath = path.join(snapshotSubDir, makeFilename());
        if (argv.capture) {
            process.stdout.write(chalk.gray(`Saving snapshot for ${chalk.white(`${test}-${snapshotIndex}`)}... `));
            ...
            process.stdout.write(chalk.gray(`Running ${chalk.white(`${test}-${snapshotIndex}`)}... `));
            ...
            if (result.doesMatch) {
                ...
                try {
                    // Ensure the failedResult directory exists
                    await Promise.all([
                        fs.writeFile(path.join('./', failedResultsDir, `${snapshotSubDirName}-${makeFilename('diff')}`), result.diffImageBuffer),
                        fs.writeFile(path.join('./', failedResultsDir, `${snapshotSubDirName}-${makeFilename('actual')}`), actualPng),
                        fs.writeFile(path.join('./', failedResultsDir, `${snapshotSubDirName}-${makeFilename('expected')}`), expectedPng),
                    ]);
                }
                catch (e) {
    ...
    await page.exposeFunction('doneTests', async () => {
        await browser.close();
        // Summarize results
        const passPerc = ((snapshotsPassed / snapshotsTested) *
            100).toFixed(1);
        const failPerc = ((snapshotsFailed / snapshotsTested) *
            100).toFixed(1);
        const skipPerc = ((snapshotsSkipped / snapshotsTested) *
            100).toFixed(1);
        if (argv.capture) {
 ...
 * @returns
 */
function compareBuffers(actualImage, expectedImage) {
    ...
    const distance = Math.sqrt(actualData.reduce((acc, cur, i) => {
        if (i % 4 === 3) {
            diffData[i] = 255;
        }
        else {
    ...
    const diffImage = upng.encode([diffData.buffer], expectedImage.width, expectedImage.height, 0);
    return {
